...
$Progcmd = 'D:\GITHUB\15Puzzle\ConsoleApp\bin\Debug\ConsoleApp.exe'
$Orders = @('RDUL', 'RDLU', 'DRUL', 'DRLU', 'LUDR',  'LURD', 'ULDR', 'ULRD')
$Heuristics = @('hamm', 'manh')
$HeuristicsDiag = @('diag', 'eucl')
...
function RunGreed([string]$heuristicdiag) {
    Write-Host '===> Strategy: greed <==='
    if ($heuristicdiag) {
        Write-Host " -> Heuristic: $heuristicdiag"
        RunProg 'greedy' $heuristicdiag
    }
    foreach($h in $HeuristicsDiag) {
        Write-Host " -> Heuristic: $h"
        RunProg 'greedy' $h
    }
}
